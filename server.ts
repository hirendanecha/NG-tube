...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/ng-tube/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(
        join(process.cwd(), 'dist/ng-tube/browser'),
        'index.html'
      )
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        if (
          ...
          const talent = {
            name: post?.title || post?.albumname || 'ng-tube Post',
            description: pdhtml?.textContent || 'Post content',
            image:
              post?.thumbfilename ||
              post?.metaimage ||
              post?.imageUrl ||
              'https://www.nudegirls.tube/assets/images/landingpage/NudeGirls-Tube-Square-Logo.png',
          };
          seo.title = talent.name;
          seo.description = strip_html_tags(talent.description);
          seo.image = talent.image;
